...
// 合并 proxy-server-nameserver：模板 + 订阅，去重
function mergeProxyServerNameserver(templateList, originalList) {
  const result = [];

  if (Array.isArray(templateList)) {
    result.push(...templateList);
  }

  if (Array.isArray(originalList)) {
    result.push(...originalList);
  }
...
function main(config, profileName) {
  ...
  const originalProxyServerNameserver =
    Array.isArray(originalDns['proxy-server-nameserver'])
