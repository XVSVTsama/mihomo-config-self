...
> 🕒 **记录时间:** 2026-06-24 09:14:40 PDT

## ✨ 核心特性 (为什么这么配？)

本配置集成了 TUN 透明代理、模块化远程规则（Rule Providers）以及精细化的应用层级分流策略，完全为满足我个人的网络环境与使用痛点而生：

* **全局 TUN **：默认开启 `tun` 模式，采用 `gvisor` 协议栈，实现全设备/全协议接管，解决部分软件不走系统代理的问题。
